...
describe('Switch CAPI Feature Flags (2.13)', {tags: '@switch'}, () => {
  ...
  before(function () {
    if (!isRancherManagerVersion('2.13')) {
      return cy.task('suiteLog', 'Test is only supported on 2.13, skipping...').then(() => {
        this.skip();
      })
    }
  })
  ...
  context('[SWITCH-TO-CAPI-PROVISIONING]', () => {
    ...
    qase(472, it('Verify ClusterctlConfig does not exist', () => {
        // This resource will not be found since it was deployed in turtlesNamespace which gets deleted on uninstalling
        // turtles chart
