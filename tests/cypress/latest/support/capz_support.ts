...
Cypress.Commands.add('createCAPZValuesSecret', (location, clientID, tenantID, subscriptionID, version?: string, registrationMethod?: string, userpoolCount = 3, systempoolCount = 3) => {
    cy.contains('local')
        .click();
    cy.get('.header-buttons > :nth-child(1) > .icon')
        .click();
